...
| `check-gitleaks-config` | **`.gitleaks.artefacts.toml`**: fails on any `[extend]` (`useDefault` would inherit gitleaks' built-in global allow-list; `path`/`url` could load one), on any allow-list (global or per rule), when a required copied default rule (`COPIED_DEFAULT_RULE_IDS`) is missing, when a copied rule's `id`, `regex`, `path`, `secretGroup`, `entropy` or `keywords` differs from `vendor/gitleaks-8.30.1-default.toml`, or when a rule is neither custom nor in that file. **`.gitleaks.toml`**: fails when it drops `[extend] useDefault = true` or loads another config, when a path allow-list entry isn't anchored with `^`, when it has a content allow-list (`regexes`, `stopwords`, `commits`), or when it defines non-custom rules. **Both**: fails when a custom rule (`azure-openai-key`, `litellm-key`, `mistral-api-key`, `groq-api-key`, `ralysa-selftest-canary`) is missing or lacks keywords or an entropy floor of at least 3, or when the two sets of custom rules differ. Also fails when the vendored default isn't the pinned file (sha256 in code and `vendor/SHA256SUMS`), and when a `.gitleaksignore` is tracked anywhere (SEC-F001-05, -24; T05-1). |
