...
// Only runs that ran to the end count (conclusion success or failure). Cancelled runs are left out
// because the PR concurrency group cancels superseded pushes, and those partial runs would pull
...
/** Conclusions of a run that executed every job to the end. */
export const COUNTED_CONCLUSIONS = ['success', 'failure'] as const;
...
/** The newest `limit` counted runs (gh lists newest first), with their wall-clock durations. */
export function runDurations(runs: readonly GhRun[], limit = DEFAULT_LIMIT): RunDuration[] {
  ...
  return runs
    .filter((run) => counted.includes(run.conclusion))
    .slice(0, limit)
    .map((run) => {
      ...
      return { id: run.databaseId, branch: run.headBranch, conclusion: run.conclusion, ms };
    });
}

export function ciDurationReport(runs: readonly GhRun[], limit = DEFAULT_LIMIT): CiDurationReport {
  const durations = runDurations(runs, limit);
