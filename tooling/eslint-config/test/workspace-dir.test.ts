...
writeFileSync(
  ...
  [
    ...
    // Results matched by filePath (absolute), not by position (review 6).
    'const byPath = new Map((await eslint.lintFiles(files)).map((r) => [r.filePath, r]));',
    'const out = {};',
    'for (const file of files) {',
    '  const config = await eslint.calculateConfigForFile(file);',
    '  const result = byPath.get(resolve(file));',
    '  if (result === undefined) throw new Error(`no lint result for ${file}`);',
    `  const errors = result.messages.filter((m) => m.ruleId === ${JSON.stringify(SYNTAX)}).length;`,
