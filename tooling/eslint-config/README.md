...
const config = [
  ...
  ...reactUi({ workspaceDir: import.meta.dirname }), // UI workspaces only
  ...tests(),
];
export default config;
```

`reactUi()` requires `workspaceDir` (the workspace folder, like `base()`'s `tsconfigRootDir`). The repo-relative workspace that places the `LOADING_EXCEPTIONS` globs is derived from it, never from the working directory, so ESLint gives the same result whether it runs inside the workspace or from the repo root.
