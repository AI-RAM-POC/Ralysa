...
- `src/commands.ts` (was `src/main.ts`, moved with `git mv`): `refuseUnsafe()` is async and awaits `openBaoStorageRefusals(config)` after the common config guards, so `migrate`, `migrate --audit`, `sealer` and `audit-verify` refuse an OpenBao whose `sys/seal-status` reports in-memory storage, is sealed, or can't be reached in 3 s. The refusal runs before `openVault()`, so nothing logs in to OpenBao or connects to the database first. Exit code 2 and the `<command> failed` JSON error line are unchanged.
