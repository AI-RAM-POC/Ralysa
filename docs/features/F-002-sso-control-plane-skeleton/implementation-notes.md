...
Branch `feat/F-002-rotation`, based on `main` after #33 (T12). T14 (scans, Dockerfile) is built in parallel on another branch, so T13 leaves `.gitleaks*.toml`, `tooling/repo-scripts`, the Dockerfiles and `.dependency-cruiser.cjs` alone.

### What landed

- **`src/secrets/runtime.ts`: the IdP client-secret watcher** (design §5.7, AC-10):
  - one per `serve` process, shared by the Graph directory and the flow-B relying party;
  - the secret is read on first use, then polled every `idp.client_secret_poll_s` (default 60); a poll adopts only a **newer** KV version, and a failed poll keeps the value in hand (`idp_client_secret_read_failed` once per failure streak, `idp_client_secret_read_recovered`);
  - `refreshAfterInvalidClient(used)`: after the IdP answered `invalid_client`, one re-read shared by concurrent callers, and a retry value only if the store now holds a version other than `used` (T13-1); `idp_invalid_client` (warn) and `idp_invalid_client_total{retried}`;
  - every adopted version is logged per replica (`idp_client_secret_observed`, gauge `idp_client_secret_version`) and recorded as `secret.rotated kind=idp_client_secret phase=observed` with an event id derived from the org, the path hash and the version, so the store keeps **one per version** across replicas and restarts (T13-4). A write that fails outright is retried on the next poll.
- `graph-directory.ts` and `oidc-client.ts` take the watcher (`clientSecret`); without one (tests) they build one over `secrets` that reads on demand. The OIDC client now caches the discovery document for an hour and rebuilds its client configuration whenever the secret version changes (T13-6). `RtsServices.idpClientSecret` passes it through `buildApp`.
- Config: `idp.client_secret_poll_s` (T13-2). Design revision 7.
- `derivedEventId()` in `src/audit/events.ts` (version 8 UUID from parts); `reportEventId` (T10-11) now uses it, with the same output.
- **TC-F-002-15 harness** `test/soak/rotation-harness.ts`, used by `test/integration/rotation.int.ts` (CI) and `test/soak/rotation.soak.ts` (TC-F-002-16). See "Tests (T13)".
- `test:soak` (`vitest.soak.config.ts`), and **`.github/workflows/soak.yml`** (`workflow_dispatch` only; T13-11).
- **Operator runbooks** in the control-plane README: rotating the RTS signing key (by the operator identity), rotating the IdP client secret with the expiry register (≤ 180 days, SEC-F002-10), and the break-glass `migrate --audit`.
- Review follow-ups R32-F1, R33-F1 and R33-F2 (below).
...
| # | Type | What | Why |
|---|---|---|---|
| T13-1 | Interpretation of §5.7 (**self-decided**) | "On `invalid_client`, re-read at once and retry once" retries **only when the re-read returns a different version** than the failed call used. Otherwise the request fails as before (Graph `unavailable` / `token invalid_client`; flow B `rejected`) with `idp_invalid_client retry=false`. A caller that failed with an older version while another caller already refreshed gets the newer value and retries. | A retry with the value that was just refused can't succeed, and it would double the requests to Entra while a secret is wrong or expired. The rotation case (KV already newer) is exactly the case the retry exists for. |
| T13-2 | Design gap, filled | `idp.client_secret_poll_s` (1–300, default 60) in `ServeConfig`. | §5.7 says "Watch (60 s)" but §3.8 had no field, and TC-F-002-15 needs compressed timings like `key_poll_s`. Design revision 7. |
| T13-3 | Implementation choice | The watcher polls `SecretStore.get()` itself and compares with the version it holds, rather than using `SecretStore.watch()`. | `watch` never reports the version it sees first. A holder that also reads on demand (first use, `invalid_client`) could adopt version N, then have `watch` start at N+1 and never be told about it. Comparing with the held version has no such gap. `watch` (T04-3) stays in the port. |
| T13-4 | Design detail (**self-decided**) | "`secret.rotated` once per version": the event id is `derivedEventId('secret.rotated', org, sha256(path), 'idp_client_secret', version)`, so a second replica's or a restarted process's write is a `duplicate`. The first version a process reads is recorded the same way (so a secret rotated while every replica was down is still recorded). It goes through `writeOrSpool`; if even that throws, it is retried at the next poll. | The design gives no mechanism. A derived id needs no coordination between replicas and no new table, and the writer already maps a primary-key conflict to `duplicate`. |
| T13-5 | SEC-F002-10 (evaluated, **not built**; self-decided) | A separate app registration for the Graph reads stays a recommendation (runbook, §6.7 checklist), not code. The config keeps one `rts_client_id` and one `client_secret_path`. | T10-18 deferred it to T13. The watcher is keyed by path, so a second registration is later a config addition (`idp.graph.{client_id, client_secret_path}` and a second watcher), with no change to the rotation logic. It needs a second confidential client in the mock IdP and a tenant admin's consent in the real one (E-1, an external blocker), and SEC-F002-10's **preferred** fix, a certificate credential signed through Transit, would replace both secrets. Listed as an open item in status.md for the founder. |
| T13-6 | Implementation choice | The OIDC client caches the discovery document for an hour (fetched with no client authentication) and builds `new Configuration(serverMetadata, …, ClientSecretPost(value))` per secret version. | T10's client re-ran discovery on a re-read and otherwise kept the secret for an hour. Now a new version from the watcher is used at once without another discovery fetch. |
| T13-7 | Implementation choice | `credential_ref_hash` for the IdP secret is SHA-256 of the KV path as configured (`kv/ralysa/control-plane/…`). The runtime logs versions, never the path or the value. | Mirrors T07-13 (`transit/<key>` for the signing key). |
| T13-8 | Scope | The design's module list also names "DB credential loading per entry point" for `runtime.ts`. Nothing is added: the pools read each role's password from KV at connect (T05, `db/pools.ts`). | Already done where the connection is made. |
| T13-9 | Test design | The TC-15/TC-16 harness runs **two replicas** over one database, rotates the **real** Transit key `ralysa-rts-signing` of the dev stack, and uses a per-run KV path. Rotation and KV writes use a token with the dev stack's `ralysa-operator` policy, and the harness checks that this identity **can't read the secret back** (403). | The design's rotation is multi-replica ("first replica wins", "once per version"). The control plane's own verifier pins `kid` to `ralysa-rts-signing.v<n>` (SEC-F002-19), so a throwaway key name can't be used for the service tokens behind the governance feed. Every other suite uses in-memory keys; the one other reader (`packages/secrets` `openbao.int.ts`) signs with the latest version it just read, which a rotation doesn't break. Each run leaves one more key version in the local dev stack; CI starts fresh. |
| T13-10 | Test design | Replica B polls the secret every 60 s while A polls every 2 s, and when the old secret is removed B's Graph directory is replaced by a fresh one sharing B's watcher. | So B still holds the old secret when the IdP drops it and must recover through `invalid_client` (flow B and Graph), under load. The swap stands in for B's cached Graph app token expiring, which otherwise happens once an hour. |
| T13-11 | Implementation choice | `soak.yml` has one job, `soak`, with the `integration` job's least-privilege shape: `permissions: contents: read`, `persist-credentials: false`, no `secrets.*`, Postgres-only container logs, artefacts kept 3 days, the pre-install gate first, and `cancel-in-progress: false`. | `check-ci-invariants` applies its general rules to every workflow (the gate first, no unconditional cancel), and they pass; its integration-specific rules only look at a job named `integration`. Naming the soak job `integration` would make a manual run report a check with the name of a required status check. Extending the rules to `soak` is a `tooling/repo-scripts` change, which T14 owns right now: open item. |
| T13-12 | Test design | TC-15 counts an **operation** as one request or one flow-B sign-in (four RTS legs plus the IdP's). The plan repeats exchange, flow B, refresh, gateway verify, refresh, control-plane verify (`/v1/me`), and each full pass goes to the other replica. The soak runs at 2 operations a second (TC-16 asks for ≥ 1). | "5 rps mixed load" doesn't say how a multi-leg sign-in counts; counting it once keeps each replica's load steady. |
| T13-13 | Constraint, recorded | TC-F-002-16 couldn't run through `soak.yml` from the branch: `workflow_dispatch` needs the workflow file on the default branch (HTTP 404). The same test ran locally and its report is in the PR; the first Actions run is a dispatch on `main` after the merge. | GitHub behaviour; adding a `push`/`pull_request` trigger to get a run would contradict the design (dispatch only). |

### Review follow-ups (from #32 and #33)

| # | Finding | Fix | Test |
|---|---|---|---|
| R32-F1 | `verifier.ts` swallowed the database error behind `VerifierUnavailableError`, and `route-auth.ts` turned it into a 503 with no log line (the key-set path from T12-4 likewise). | `VerifierUnavailableError(message, { cause })` in `@ralysa/auth`; the package passes the jose or key-set fault, and `createDbRevocationSource` passes the database error. `authenticate()` logs one warn line `verifier_unavailable` with `errorSummary(error.cause)` (type, scrubbed message, code) and `token_kind` before the 503. | `verifier.test.ts`: a revocation read fault and a key-set fault each write exactly one `verifier_unavailable` warn line with the cause's summary; a fault message that carries the bearer token is scrubbed, and neither the token nor its payload segment appears in any line. `service-token-verifier.test.ts`: the cause is carried, the message isn't. |
| R33-F1 | R33-9 (keyset paging and commit visibility) was only in the notes. | One sentence in the README's audit query section and in the `auditQuery` route summary; OpenAPI regenerated (`check:generated` clean). | `openapi.test.ts` (committed document = generator output). |
| R33-F2 (R33 nit 1) | `withAuditLocked` counted lock waiters only, so a refused insert still between statements could commit after the `ROLLBACK`. | It also waits until no `ralysa_audit_writer` backend of the test database is anything but `idle` (so none is running or inside a transaction) before releasing the lock. | `audit-routes.int.ts` passes (32). |

### Tests (T13)

- **Unit** (control-plane 26 files, 383 tests; 14 new):
  - `idp-client-secret.test.ts` (10, fake timers only): one read for concurrent first callers, then memory; a poll adopts a newer version and never an older one; `invalid_client` with one shared re-read, a retry only with a newer version, and the retry for a caller that failed with an older version; `secret.rotated observed` once per version with the same derived id on two replicas (a version 8 UUID; another org gives another id); a failed record retried on the next poll; OpenBao down keeps the value with one warn per streak, then recovers; a failed first read rejects and the next call reads again; `stop`; no value or path in any log line or event.
  - `graph-directory.test.ts` (+2): no retry when the store has no newer secret (one token request); the shared watcher's value is used.
  - `verifier.test.ts` (+2) and `@ralysa/auth` `service-token-verifier.test.ts` (+1): R32-F1.
- **Integration, TC-F-002-15** (`test/integration/rotation.int.ts`, CI `integration` job): 60 s at 5 operations a second over two replicas, key rotated at 15 s and the IdP secret at 20 s, `key_poll_s` 2, `activation_delay_s` 6. Local run (2026-09-26):
  - 297 operations (50 exchanges, 99 refreshes, 50 flow-B sign-ins, 49 gateway and 49 control-plane verifications), **0 failures**;
  - the new kid (`ralysa-rts-signing.v5`) in use **7.0 s** after the rotation (bound 2 × 2 + 6 = 10 s, asserted ≤ 12 s); 20 of 20 old-kid tokens verified at the end; JWKS listed both kids;
  - `secret.rotated`: `published` and `activated` once for v5; `observed` once for IdP secret v1 and once for v2;
  - replica A held v2 **1.9 s** after the `kv put`; replica B got it 3.2 s after, through **one** `invalid_client` re-read and retry, after the old secret was removed at 22 s; no warn or error line other than B's `idp_invalid_client`.
- **Soak, TC-F-002-16, on Actions (the G6 condition, R35-3):** `soak.yml` dispatched on `main` at `ee80613` after #35 merged, run [36204987424](https://github.com/AI-RAM-POC/Ralysa/actions/runs/36204987424), **passed** (job 00:28:43–00:39:35 UTC, 2026-09-26). 1,199 operations (200 exchanges, 400 refreshes, 200 flow-B sign-ins, 200 gateway and 199 control-plane verifications), **0 failures**; key rotated at 60 s and the new kid (`ralysa-rts-signing.v2`) in use **150.6 s** later (bound 180 s, AC-10 300 s); 20 of 20 old-kid tokens verified at the end; IdP secret v2 held by replica A after **59.9 s** and replica B after **60.0 s**, no `invalid_client`; `secret.rotated` once per phase and version; no warn or error line.
- **Soak, TC-F-002-16, earlier local run**: run once **locally** on 2026-09-26 (`pnpm --filter @ralysa/control-plane test:soak` on the dev stack), because GitHub dispatches only workflows that exist on the default branch: `gh workflow run soak.yml --ref feat/F-002-rotation` answered `HTTP 404: workflow soak.yml not found on the default branch` (T13-13). 600 s at 2 operations a second, `key_poll_s` 30, `activation_delay_s` 120, both replicas polling the secret every 60 s, the old secret removed only once both replicas reported the new one (the runbook):
  - 1,196 operations (200 exchanges, 398 refreshes, 200 flow-B sign-ins, 199 gateway and 199 control-plane verifications), **0 failures**;
  - key rotated at 60 s; the new kid (`ralysa-rts-signing.v6`) in use **150.1 s** later (bound 2 × 30 + 120 = 180 s; AC-10 allows 300 s); 20 of 20 old-kid tokens verified at the end;
  - IdP secret v2 written at 120 s; replica A held it after **59.9 s**, replica B after **60.0 s** (≤ 300 s); the old secret was removed at 180 s; no `invalid_client` at all;
  - `secret.rotated`: `published`/`activated` once for v6, `observed` once for secret v1 and v2; no warn or error line.
  - The report file (counts, timings, kids and versions only) is attached to the PR.
- `pnpm repo:check`: `check-ci-invariants` passes with `soak.yml`. A copy of `soak.yml` with `pnpm` before the gate and `cancel-in-progress: true` gives `ci/pre-install-gate-first` and `ci/cancel-in-progress`, so the file is covered (checked by hand, not committed: `tooling/repo-scripts` is T14's).

### Open items (T13)

- `check-ci-invariants`: apply the `integration` job's least-privilege and artefact rules to the `soak` job too (T13-11). It belongs in `tooling/repo-scripts`, after T14.
- SEC-F002-10: a separate Graph app registration, or the preferred certificate credential through Transit (T13-5). Needs the founder's call and E-1.
- ~~Signing keys (T07 behaviour, seen in the harness): a Transit version that is published but never activated is never superseded, so it never retires and stays in JWKS.~~ Fixed in this PR (R35-T07 below).

### Checks (T13 definition of done)

- `pnpm exec turbo run lint typecheck test build check:generated --force`: 97/97 tasks (Node 24.21.0). Control-plane unit 383, `@ralysa/auth` 119.
- `RALYSA_REQUIRE_DEV_STACK=1 turbo run test:integration`: 13/13 tasks; control-plane 11 files, 156 tests (with TC-F-002-15), `@ralysa/secrets` 5, dev-stack 66.
- `pnpm repo:check`: all checks pass, including `check-ci-invariants` with `soak.yml` and `check-integration-scope` with `rotation.int.ts`.
- `secret-scan pr` over the branch: 0 findings.

### Code review of PR #35 (changes requested, one blocker): resolutions

| Item | Finding | Fix | Test |
|---|---|---|---|
| R35-1 (blocker) | Any signed-in user or registered service could pre-empt `secret.rotated observed`: its v8 event id is derived from guessable inputs, `audit_event.event_id` is a global key, and both ingest routes accepted any UUID. An event stored first under that id made the server's insert a `duplicate`, treated as recorded. | The external ingest schemas accept only a lower-case **version 7** `event_id`: new `UuidV7` in `@ralysa/protocol/common`, `ServiceAuditEventInput` (= `AuditEventInput` with a v7 id) for `ServiceEventsRequest`, and `ClientAuditEventInput.event_id`. The stored envelope `AuditEventInput`, which the writer validates, still accepts the server's v8 ids. Schemas and OpenAPI regenerated; design revision 8; README. **Audit of server-derived ids:** `derivedEventId` (`secret.rotated observed`, and `reportEventId` for sign-in failure reports) and `sessionEventId`/`gapEventId` (client-session and `audit.client_seq_gap` events) are all version 8, so none can now be submitted by a client or a service. Events generated in the database (`audit.modify_denied`, `audit.schema_changed`) use random v4 ids and are never submitted through ingest; every other server event uses a fresh v7. The service rejection reporter in `@ralysa/auth` already sends v7 ids. | `@ralysa/protocol` `control-plane.test.ts`: v8, v4 and upper-case v7 ids refused by both requests; the stored envelope still accepts a v8 id. `audit-routes.int.ts`: a service event and a client event carrying the precomputed `secret.rotated` id are 422 and nothing is stored under it. |
| R35-2 | The harness's "the operator can't read the secret" check ran before the first `kv put` and accepted 404, so it always passed. | It runs after the first `kv put` and requires exactly 403. | TC-F-002-15 (the harness throws otherwise). |
| R35-3 | T13's definition of done asks for TC-F-002-16 through `workflow_dispatch`, linked in the PR; it can't be dispatched from a branch (T13-13). | Recorded in status.md as an explicit **G6 condition**: dispatch `soak.yml` on `main` after the merge and link the run in the test report. | — |
| R35-4 | A KV version that went backwards was silently ignored. | The watcher keeps its value and logs `idp_client_secret_version_regressed` (`held_version`, `store_version`) once per streak, with `idp_client_secret_version_regressed_total`; the streak ends when the store reaches the held version. Runbook: never delete the KV metadata or recreate the path; if a version goes backwards, write the right value and restart every replica. | `idp-client-secret.test.ts`: one warning for a streak of polls, a second one after the streak ended and started again, no value in the lines. |
| R35-n1 | The secret reads on the flow-B redeem path were outside the 3 s bound (the OpenBao client's timeout is 5 s). | `current()` and `refreshAfterInvalidClient()` in the OIDC client are raced against `OIDC_TIMEOUT_S`; a timeout answers `unavailable client_secret`. | `oidc-client.test.ts` (fake timers): a hanging read ends at exactly 3 s, before any IdP call. |
| R35-n2 | A failed re-read after `invalid_client` wasn't counted or logged. | `idp_client_secret_read_failures_total{via}` and the once-per-streak `idp_client_secret_read_failed` (now with `via`) for both the poll and the re-read; the error still reaches the caller. | `idp-client-secret.test.ts`. |
| R35-n3 | The README promised the `idp_client_secret_version` gauge, which is a no-op until `serve` has a metrics exporter (R29-n5). | The README says so and points at the log line. | — |
| R35-n4 | The runbook's `az ad app credential reset` printed the new password. | The password goes into a shell variable (`--query password -o tsv`), is written to KV with `printf … | bao kv put … value=-`, and the variable is unset; the key id comes from `az ad app credential list`. A direct pipe was not used because the new credential needs a few minutes to propagate before replicas may adopt it. | — |
| R35-n5 | `audit-verify` needs `--config`. | Both runbook commands pass `--config <audit-verify config>`. | — |
| R35-n6 | The §5.7 diagram said "retry once" without the newer-version condition. | "retry once (only if the KV version is newer)". | — |
| R35-n7 | A throwing harness operation was reported as `exchange`. | The plan pairs each operation with its kind, and a throw is reported under that kind. | — |
| R35-n8 | `verifier_unavailable` was written for every request during an outage. | At most one line per route template per 10 s (`VERIFIER_UNAVAILABLE_LOG_EVERY_MS`), per app; the next line carries `route` and `suppressed` (lines skipped since). | `verifier.test.ts` (fake `Date` only): three failures on one route and one on another give two lines; after the window the next line has `suppressed: 2`. |
| R35-T07 (T07 follow-up, separate commit) | `signing-keys.ts` superseded only versions that were once active, so a version published but never activated (several at the first start, or two rotations within one poll) never retired and stayed in JWKS (the soak ended with v1–v6 listed). | On activation, every lower live version without `superseded_at` is superseded (`versionsToSupersede()`, the pure rule), so it retires after the usual retention with `secret.rotated phase=retired`. The UPDATE is also scoped to this key's kids. Design revision 8. | `signing-keys.test.ts`: three versions at the first poll (v3 active, v1 and v2 superseded, gone from `jwksRows` after retention); two rotations within one poll; newer, superseded and retired rows untouched. `serve.int.ts`: a Transit key with three versions at the first poll: v1 and v2 leave JWKS after the retention with one `retired` event each. |

Checks after the review: `turbo run lint typecheck test build check:generated` 97/97 (control-plane unit 390, `@ralysa/protocol` 171, `@ralysa/auth` 119); `test:integration` 13/13 (control-plane 159, including TC-F-002-15); `pnpm repo:check` passes.

### Follow-ups after #35 merged (branch `fix/F-002-signing-key-pin`, issue [#36](https://github.com/AI-RAM-POC/Ralysa/issues/36))

| Item | Finding | Fix | Test |
|---|---|---|---|
| R35-F1 (T07, #36) | Rolling back with `tokens.signing_key_pin_version` stopped signing about one access-token lifetime later: the retire loop didn't skip the pinned version, which was already superseded, so it retired, `selectActiveVersion` returned nothing, minting failed and `/readyz` went unready. | New pure `versionsToRetire()` never returns the pinned version, and a pinned version that had been superseded has `superseded_at` cleared while pinned, so it stays in JWKS and keeps signing. Supersede now runs on every poll (guarded by `superseded_at IS NULL`), so once the pin is removed the newest active version supersedes the former pin, which then retires after the retention. A version already retired can't be pinned (unchanged). ~~While pinned, the newer version stays in JWKS (its tokens verify until they expire).~~ **Revised in the review of #37 (R37-1):** while pinned, every newer version that was ever active is superseded too, so it leaves JWKS after the retention even though the pin holds, and whichever version is selected never carries `superseded_at` (`versionToUnsupersede()`). README runbook updated. | `signing-keys.test.ts`: retirement after the retention only; the pinned version never retired (and still selected); retired and never-superseded rows skipped. `serve.int.ts`: pin v1 after v2 took over, wait past the retention, v1 still signs (`kid …v1`) and is in JWKS; unpin, v2 signs, v1 retires with one `retired` event. Against the old `signing-keys.ts` the integration case fails. (Both cases were rewritten in the review of #37: v2 now retires while pinned.) |
| R35-F2 (nit) | `poll()` ran its own SQL while `versionsToSupersede()` was used only by tests. | `poll()` supersedes exactly `versionsToSupersede(rows, next)` (by kid) and retires exactly `versionsToRetire(rows, now, timing)`. | The unit tests of both helpers now cover the rule `poll()` applies; the `serve.int.ts` cases exercise it. |
| R35-F3 (nit) | README step 2 used `&& unset NEW_SECRET`, which keeps the variable if the write fails. | `; unset NEW_SECRET`. | — |

TC-F-002-16 on Actions: [run 36204987424](https://github.com/AI-RAM-POC/Ralysa/actions/runs/36204987424), passed (see "Tests (T13)"). Checks on this branch: control-plane unit 393; `test:integration` 13/13 (control-plane 160); `pnpm repo:check` passes.

### Code review of PR #37 (changes requested, one blocker): resolutions

Branch `fix/F-002-signing-key-pin`, `main` (T14, #34) merged in. Every decision below was self-decided under the standing authorization (CLAUDE.md, 2026-09-25) by taking the recommended option, recorded by Claude; design revision 9.

| Item | Finding | Fix | Test |
|---|---|---|---|
| R37-1 (blocker) | After pinning v1 because v2 was bad, `versionsToSupersede(rows, 1)` superseded only versions below 1, so v2 stayed in JWKS (its tokens verifiable) for as long as the pin held, contrary to §9 ("stay verifiable until they expire") and §3.2.4 (superseded + max access TTL + 5 min). | Option (b) from the review. `versionsToSupersede(rows, selected, pinVersion)` also returns every live version **above** the pin that was ever active, so v2 is superseded on the first pinned poll and retires after the retention while the pin still holds. A newer version that was only published is left alone (it activates once the pin is removed). The un-supersede is generalised: the **selected** version never carries `superseded_at`, pinned or not, which avoids the mirror of #36 (unpinned before v2 retired, v2 selected but superseded, so it would retire while signing). On unpin before v2 retired: v2 is selected, un-superseded, and v1 is superseded again on the same poll and retires after the retention. On unpin after v2 retired: v1 is the newest live version, keeps signing, nothing changes. `versionsToRetire(…, selected)` never returns the selected version either. Design §3.2.4 and §9 (revision 9), README runbook (keep the pin until the bad version has retired; a good version rotated in while pinned doesn't help before its activation delay has passed, because the newest active version is the bad one). | `signing-keys.test.ts` "versionsToSupersede while pinned": the newer active version superseded and retired after the retention, a never-active one left alone, nothing above the selection when it is not the pin, retired or already superseded rows untouched. `serve.int.ts`: (1) pin v1, past the retention v1 signs and v2 has left JWKS, then unpin and v1 keeps signing; (2) pin, unpin within the retention: v2 signs (`kid …v2`), v1 retires. Both fail against the previous `signing-keys.ts`. |
| R37-2 (nit) | No audit event when a pinned version is made live again. | The un-supersede UPDATE is guarded (`superseded_at IS NOT NULL AND retired_at IS NULL … RETURNING`) and only the replica whose UPDATE changed the row writes `secret.rotated` with phase `pinned` (the pinned version) or, **self-decided**, `reactivated` (a newer version selected again after the pin is removed: signing moving back to it is as significant as the pin). Log line `signing_key_unsuperseded`. The phases aren't enumerated in `@ralysa/protocol` (`details` is free-form there), so only design §3.5's catalogue gains them. Pinning a version that was never superseded changes no row and records nothing (also self-decided: the event records a key-state change, and the pin itself is a config change). During a rolling restart that changes the pin, replicas disagree and the events may repeat until the rollout ends (README, design §3.2.4). | `serve.int.ts` expected sequences: published/activated v1 and v2, then `pinned` v1 and `retired` v2 (case 1); `pinned` v1, `reactivated` v2, `retired` v1 (case 2, with a second pinned poll that records nothing twice). |
| R37-3 (nit) | The retire UPDATE trusted the retention check made on the rows read at the start of the poll. | The UPDATE also requires `superseded_at IS NOT NULL AND superseded_at <= clock_timestamp() - make_interval(secs => retention)`, so a version un-superseded (or superseded again) since the read is left alone. | `serve.int.ts` retirement cases (the retention and the SQL check agree at 1 s). |
| R37-4 (nit) | The un-supersede rule was inline in `poll()`. | Pure `versionToUnsupersede(rows, selected)` next to `versionsToSupersede`/`versionsToRetire`; `poll()` applies only the three helpers. | `signing-keys.test.ts` "versionToUnsupersede": pinned and superseded; unpinned before and after the newer version retired; the runbook case (a good version rotated in while pinned); not superseded, retired or nothing selected. |
| R37-5 (nit) | The header comment of `signing-keys.ts` still said the replica that activates supersedes. | Rewritten: supersede runs on every poll from any replica (guarded), then pin, un-supersede and retire as they now run. | — |
| R37-6 (nit) | status.md line 3 still said "T13 in review". | T13 merged (#35), T14 merged (#34), #37 in review, T15 next; the T13-11 open item (CI invariants for `soak`) is closed by #34 (R34-n6). Follow-up nits from #34 are tracked in [#38](https://github.com/AI-RAM-POC/Ralysa/issues/38). | — |

Checks after the review (Node 24.21, `main` at `c5c6916` merged in): `turbo run lint typecheck test build check:generated --force` 97/97 (control-plane unit 402, `@ralysa/protocol` 171, `@ralysa/auth` 119, `@ralysa/repo-scripts` 735); `turbo run test:integration --force` 13/13 (control-plane 12 files, 167 tests; dev-stack 66; secrets 5); `pnpm repo:check` passes. Two transient failures were seen and did not recur on re-run: one cached-pipeline run failed `@ralysa/control-plane#test` without output kept (the next two runs, one uncached, passed 402/402), and one `test:integration` run failed `audit.int.ts` "concurrent sealers never fork a chain" with `AuditUnavailableError: write exceeded 250 ms` while three packages' integration suites ran in parallel against the same dev stack (not code this PR touches; the control-plane suite alone and the next full run passed).
...
- **dependency-cruiser rule `no-dev-only-in-shipped`** (SEC-F002-13 a): `@ralysa/dev-stack`, `oidc-provider` or a relative path into `tooling/dev-stack/`, imported from anywhere outside `tooling/**` and a workspace's own `test/` folder (anchored per workspace since R34-2). `boundaries.js` gains `DEV_ONLY_IMPORT_ALLOWED_IN` and `DEV_ONLY_MESSAGE` beside `DEV_ONLY_PACKAGES`, which it documents as the list for all four layers.
...
### Code review of PR #34 (changes requested, no blockers): resolutions

| Item | Finding | Fix | Test |
|---|---|---|---|
| R34-1 (S1) | `.dependency-cruiser.cjs` **excluded** build output. Once `tooling/dev-stack/dist` was built, `import '@ralysa/dev-stack/mock-idp'` from shipped code resolved through `exports` into the excluded `dist`, so the import disappeared and no rule fired. | Build output (`dist`, `coverage`, `.turbo`, `.tsc` in our folders) moved from `exclude` to `doNotFollow`. dependency-cruiser doesn't cruise it as a source (initial-source gathering skips `doNotFollow` too), but it stays a **target**. Nothing is excluded now. The real repository, with every `dist` built, still passes `check-imports`. | `check-imports.test.ts` "R34-1": a fixture with a built `tooling/dev-stack/dist`, its `exports`, and a pnpm-style symlink in the dependant's `node_modules`. The import fails and the message names the `dist` file. With the old `exclude` put back, this test fails (checked). |
| R34-2 (S2) | `DEV_ONLY_IMPORT_ALLOWED_IN` `'**/test/**'` also allowed `services/*/src/test/**`, which is compiled into `dist` and shipped. | Anchored per workspace: `tooling/**`, `apps/*/test/**`, `packages/*/test/**`, `services/*/test/**`, `packs/**/test/**`. | "R34-2": `services/control-plane/src/test/helpers.ts` and `packages/auth/src/lib/test/fake.ts` fail; `packages/auth/test/flows.test.ts` passes. |
| R34-3 (S3) | `check-no-password` missed several forms: `passcode`/`passCode`, a bare `pwd`, camelCase identifiers (`userPin`, `otpCode`, `totpCode`; `\b` doesn't split them), `sharedSecret`, `'Secret: '` prompts, and code after a block comment on the same line. The OpenAPI walker skipped `description`/`title`/`summary` even when it was a property **named** `description`, and didn't check `default`, `pattern` or `example` values. | <ul><li>Every name is matched as written **and** with camelCase, `_` and `-` split into words (`matchForms`).</li><li>The word rule uses letter lookarounds instead of `\b` and adds `pass[ _-]?code`, `pwd`, `(shared\|user)[ _-]?secret` and secret prompts (`"Secret:"`, `"Enter your secret?"`). The syntax rules (input type, grant type) match the line as written only.</li><li>A line is a comment only if nothing follows the closing `*/`, `*/}` or `-->`.</li><li>OpenAPI: prose keys are skipped only when their value is a string. Every string anywhere under `name`, `enum`, `const`, `default`, `pattern`, `example` and `examples` is checked, and names are matched in both forms.</li></ul> The real repository still passes, with no hits to fix or justify. | `check-no-password.test.ts`, 42 tests (21 new): 14 client fixtures (passcode, passCode, pwd, userPin, otpCode, totpCode, user_pin, sharedSecret, user_secret, two secret prompts, and code after a block comment, a JSX comment and a comment continuation); 6 OpenAPI fixtures (a property named `description`, `userPin`, `default`, `pattern`, a nested `example`, `examples`); look-alikes stay clean (`spinner`, `pinned`, `hotpath`, `opinion`, `secretary`, `passport`, and whole-line comments). |
| R34-4 (S4) | `# syntax=docker/dockerfile:1` fetched a floating frontend image at build time. | Dropped. The builder's built-in frontend covers everything the file uses, and the header comment says why the directive is absent. | The image builds and scans clean locally (0/0/0). CI builds it in `integration`. |
| R34-n1 | `ci/integration-image-scan` accepted a step with `if: false` (or any condition) or `continue-on-error: true`. | The step counts only with no `if` or `if: ${{ !cancelled() }}`, and without `continue-on-error: true`. The job may not set `continue-on-error: true` either. | `check-ci-invariants.test.ts`: `if: false`, another condition, step `continue-on-error`, and job `continue-on-error` each fail. A step without `if` still counts. |
| R34-n2 | `DEV_ONLY_IN_IMAGE` duplicated `DEV_ONLY_PACKAGES`. | **Kept the copy, with an equality test (self-decided).** `secret-scan-image.ts` is loaded by `secret-scan-cli.ts`, which the no-install `secret-scan` job runs with plain node, so it can't import `@ralysa/eslint-config`. | `secret-scan-image.test.ts`: `DEV_ONLY_IN_IMAGE` equals `DEV_ONLY_PACKAGES`. |
| R34-n3 | `SECRET_NAME` missed `*_KEY` names such as `SIGNING_KEY`. | KEY as a word of the name (`(^\|[_-])KEY($\|[_-])`), plus `APIKEY`/`ACCESSKEY`. Also fixed a latent miss: `PASSWORD_FILE` didn't match, because `\b` doesn't split `D_`. | 13 names that must match (`SIGNING_KEY`, `KEY`, `KEY_FILE`, `AUDIT-HMAC-KEY`, `PASSWORD_FILE`, …) and 7 that must not (`PATH`, `NODE_VERSION`, `YARN_VERSION`, `NODE_ENV`, `MONKEY`, `KEYBOARD_LAYOUT`, `HOME`). |
| R34-n4 | The post-prune check loaded only the serve graph. | The build stage also runs `node dist/main.js --help` and requires its usage line. `main.js` statically imports every entry point (serve, bootstrap-org, migrate, sealer, audit-verify), so a pruned module that any of them needs fails the build with a stack trace instead of the usage line. | The local image build ran the step. |
| R34-n5 | `.dockerignore` lacked `.envrc`, `.ssh`, `.aws` and SSH key files. | Added `**/.envrc`, `**/.ssh`, `**/.aws`, and SSH keys by their standard names (`**/id_rsa*`, `id_dsa*`, `id_ecdsa*`, `id_ed25519*`). **Self-decided:** not `**/id_*`, because it would also drop a source file such as `id_token.ts` from the build. No tracked file matches the new patterns. | — |
| R34-n6 | The locked-down job rules applied only to a job named `integration`. T13's `soak.yml` (#35) has a `soak` job of the same shape. | `LOCKED_DOWN_JOBS` (`integration` with the image scan, `soak` without), matched by job id in any workflow. The rule ids stay `ci/integration-*` so existing references hold. #35's `soak.yml` was read (not edited): it meets every rule (`contents: read`, `persist-credentials: false`, no `secrets.*`, Postgres-only logs, `retention-days: 3`), so the check takes effect when #35 lands. | "the locked-down rules apply to a `soak` job too": the integration job renamed `soak`, without the image step, passes; `contents: write` fails `ci/integration-no-secrets`; logs of every service fail `ci/integration-artefact`. |
| R34-n7 | **CI finding after the review fixes.** The first CI run of the fixes failed `integration` on an unhandled `57P01` ("terminating connection due to administrator command") attributed to `checkpoint.int.ts`. `drop()` ends the pools, then `DROP DATABASE … WITH (FORCE)` terminated an `audit_migrator` client that was still closing, and its late `error` event had no listener. Vitest also printed the client object into the CI log, including that run's generated role password. That value is throwaway: it is created per run and dies with the job's dev stack. | `test/integration/support/db.ts`: every test pool, and each client it connects, gets an `error` listener, so a late termination is ignored. Query errors still reject their promises. This was a pre-existing race in the shared harness, made more likely by one more parallel file. | The full control-plane integration suite locally (11 files, 161 tests), then CI. |
| R34-n8 | **CI finding, second run.** In `audit-routes.int.ts` (T12), "no token → 401; …" read the `auth.token_rejected` baseline **after** the 401 request. That request's `malformed` rejection is written fire-and-forget, so it could already be in the baseline, and the test then saw 2 new events instead of 3. | The baseline is read after `settled()` and **before** the first request. This was a pre-existing race in the test, not in the route. | CI. |
...
## Fix for #41: the OpenBao storage refusal at every entry point

Branch `fix/F-002-openbao-refusal-all-entry-points`, based on `main` at `e6e6b98`, PR [#42](https://github.com/AI-RAM-POC/Ralysa/pull/42). Resolves T15-2 and R40-1, the G6 entry condition on TC-F-002-34 (SEC-F002-12, threat T-12).

### What changed

- `src/commands.ts` (extracted from `src/main.ts`; git records it as a new file): `refuseUnsafe()` is async and awaits `openBaoStorageRefusals(config)` after the common config guards, so `migrate`, `migrate --audit`, `sealer` and `audit-verify` refuse an OpenBao whose `sys/seal-status` reports in-memory storage, is sealed, or can't be reached in 3 s. The refusal runs before `openVault()`, so nothing logs in to OpenBao or connects to the database first. Exit code 2 and the `<command> failed` JSON error line are unchanged.
- `src/serve.ts`: `bootstrapOrgCommand` awaits the same check next to `serveProductionRefusals`, as `serveCommand` does.
- `src/main.ts` is now only the executable (`process.exitCode = await main(...)`); `commands.ts` holds `main()` and the commands and has no side effects on import, so a unit test can drive `main()` in process. The bin, the image entry point and `node dist/main.js --help` are unchanged.
- `test/entry-point-guards.test.ts` (TC-F-002-34): one case per entry point (`serve`, `bootstrap-org`, `migrate`, `migrate --audit`, `sealer`, `audit-verify`). Each writes a hardened production config (the config guards pass), stubs `fetch` with `storage_type: inmem`, and asserts that `main()` returns 2, the only fetch is the seal-status probe, and the error line names the in-memory refusal. Two more cases: a config refusal and the OpenBao refusal are reported together, and the check isn't made in `dev` or `test`. With the fix removed the per-entry-point cases fail (checked locally).
- `README.md` production guards: the OpenBao check moves to "every entry point"; the "`serve` only" item keeps the signing-key custody check; the note about the gap is replaced by why the check matters with an overridable `vault.addr`, and the fail-closed behaviour.
...
| # | Type | What | Why |
|---|---|---|---|
| F41-1 | Scope (**self-decided**) | The refusal applies only when `env=production`, exactly as in `serve`: `openBaoStorageRefusals()` itself returns no refusals outside production and is reused unchanged. Its sealed and unreachable refusals come with it at every entry point. | §3.8 scopes every production guard to `env=production`; the dev stack's OpenBao is a dev server, so a dev-wide check would stop every dev and CI run. One function keeps the entry points identical. Failing closed on an unreachable OpenBao costs nothing extra: each entry point needs OpenBao for its credential (and the sealer and `audit-verify` for Transit) straight after. |
| F41-2 | Scope (**self-decided**) | `audit-verify` gets the check too, though it reads only the checkpoint key's public keys and the `audit_reader` credential. | Design §3.8 says every entry point. Its trust anchor is the live OpenBao key (SEC-F002-36): a dev OpenBao behind an overridden `vault.addr` could serve public keys for checkpoints it signed itself, which is the case R40-1 names. Every entry point has `vault.addr` in its config (`Common.vault`), so each can make the check. |
| F41-3 | Implementation choice (**self-decided**) | `main.ts` is split into the executable (`main.ts`) and the commands (`commands.ts`). | `main.ts` ran `main(process.argv)` on import, so a test couldn't import it. Guarding the call with an `import.meta.url` check would depend on how the process was started (bin symlink or `node dist/main.js`); a separate module changes nothing at run time. |
| F41-4 | Test scope | No integration test was added. TC-F-002-34 is a unit test case in the design (§8), and no integration test covered `serve`'s refusal to extend. | The unit tests drive each real entry point through `main()` with only `fetch` stubbed; the seal-status request and response parsing are the existing `openBaoStorageRefusals()` tests. |
...
| R58-D4 | Behaviour (**self-decided**) | Sign-in always writes its own Graph answer, and stamps `graph_checked_at` with its write time (`greatest`), not its Graph-call time. | A sign-in is a fresh, complete check. The only imprecision is a refresh whose Graph call started during the sign-in's own Graph call (≤ 3 s): that refresh is treated as older. That errs toward an answer at most seconds older, never toward a stale answer that was overtaken. |
...
| R58-D7 | Gap (**flagged, not decided**) | The runbook's "revoke the user's Ralysa sessions (G-1)" has no operator tool in Phase 0. Admin-initiated revocation is F-006 (REQ-017). The runbook says so, lists the fastest paths that exist (the user's own sign-out; Entra disable or revoke-sessions, which acts at the next refresh), and forbids unaudited manual SQL. | An operator `revoke-user` command or API would be a new side-effecting admin action. That needs a design and an approval hook, so it isn't built here. It is raised for the founder: build it in F-002, or keep it in F-006. |
