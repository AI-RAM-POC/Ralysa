...
- **Soak, TC-F-002-16**: run once **locally** on 2026-09-26 (`pnpm --filter @ralysa/control-plane test:soak` on the dev stack), because GitHub dispatches only workflows that exist on the default branch: `gh workflow run soak.yml --ref feat/F-002-rotation` answered `HTTP 404: workflow soak.yml not found on the default branch` (T13-13). 600 s at 2 operations a second, `key_poll_s` 30, `activation_delay_s` 120, both replicas polling the secret every 60 s, the old secret removed only once both replicas reported the new one (the runbook):
