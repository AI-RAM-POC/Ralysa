...
- `deploy/docker/dev/compose.yaml`: the `mock-idp` service (profile `idp`). It runs `node:24.21.0-alpine` pinned by digest and executes `dist/mock-idp/main.js` from a read-only bind mount of the repository. It runs as a non-root user with a read-only root filesystem, `cap_drop: ALL` and a tmpfs `/tmp`. Only 127.0.0.1:59400 is published, and there is a discovery healthcheck. Its ids match `control-plane.serve.dev.yaml`.
