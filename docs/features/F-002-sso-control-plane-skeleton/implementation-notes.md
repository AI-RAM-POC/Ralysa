...
- Signing keys (T07 behaviour, seen in the harness): a Transit version that is published but never activated (several versions present at the first start, or two rotations within one poll) is never superseded, so it never retires and stays in JWKS. Harmless for verification (Transit keeps it non-exportable and nothing signs with it), but JWKS keeps trusting a key that was never used. Proposed follow-up: supersede every lower version when a newer one activates.
