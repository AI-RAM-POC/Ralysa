...
| 2026-09-26 | 10 | Fix for [#47](https://github.com/AI-RAM-POC/Ralysa/issues/47) (SEC-F002-42, founder decision C2: fixed before G7), self-decided details under the standing authorization, recorded by Claude. **The `Principal` contract is clarified.** §3.4.2: `roles` are *directory* roles (current memberships of the configured groups), never enough on their own for `platform_admin`; `GET /v1/internal/principals/{user_id}?sid=` adds `session_id` and `session_roles` = the session's roles ∩ current memberships, and refuses (403) an unknown, another user's, revoked, pending or expired `sid`. PEPs authorize on `session_roles` (§3.6, §5.5, §6.1). "Current memberships" and the §3.4.6/§6.1 "re-read at request time" mean: the memberships Graph last confirmed at a sign-in **or refresh**, read from the database at request time; the refresh grant now writes Graph's answer for the two configured groups back to `cp.group_membership` (`graph_check`) and audits a change (§3.5, §4.4, §5.3). `serve` reconciles `cp.idp_group.role` with `access.*` at start and writes the new `directory.group_role.changed` per change (§3.5, §4.4). §3.6: `resolve(userId, sessionId)`, cached per (user, session), `PrincipalSessionRefusedError`. Control-plane API 1.1.0 (§3.10: optional fields only). **PR #58 review round 2** (R58-1..5, SEC-F002-52..55): only the `serve` start changes group roles, serialized per org by an advisory lock and reporting only rows it changed; sign-in creates a missing configured row and logs `group_role_config_skew`, never rewriting a role. Graph write-backs are ordered by when their Graph call started (`cp.app_user.graph_checked_at`, migration cp/0007, per-user advisory lock), so an older answer never overwrites a newer one. The one-argument `resolve(userId)` is deprecated. §3.8: `access.access_group_id` and `access.admin_group_id` must differ. See implementation-notes.md "Fix for #47". |
...
- A superseded version stays in JWKS until `activated_at(next) + max access TTL + 5 min`, so tokens signed with it validate until they expire (AC-10). When a version activates, every lower live version is superseded, including one never active (rev 8).
...
- Emergency pin: `tokens.signing_key_pin_version` in config forces a known-good version (rollback, §9). While pinned (rev 9): the pinned version signs, is never retired and never carries `superseded_at` (cleared if it had been superseded, `secret.rotated phase=pinned`); every **higher** version that was ever active is superseded as well, so the bad version stays in JWKS only for the usual retention (max access TTL + 5 min from the pin taking effect) and then retires, even though the pin still holds. A higher version that was published but never activated is left alone. Whatever version is selected never carries `superseded_at`: if the pin is removed before the newer version retired, that version is selected again, its `superseded_at` is cleared (`phase=reactivated`) and the former pin is superseded and retires after the retention; if the newer version has already retired, the former pin stays the newest live version and keeps signing. Every change is a guarded UPDATE, and the replica whose UPDATE changed the row writes the event. During a rolling restart that changes the pin, replicas with the old and the new setting disagree until the rollout ends: both versions stay in JWKS and the `pinned`/`reactivated` events may repeat.
...
export const ServeConfig = z.strictObject({
  ...
  idp: z.strictObject({
    ...
    client_secret_poll_s: z.int().default(60),   // the secret watcher's poll (§5.7) [rev 7]
...
| .graph_checked_at | timestamptz null: when the Graph call behind the last membership write-back started (rev 10, cp/0007); an older answer is skipped | N | | |
...
| .role | text null (`access`, `platform_admin`), set only from config; changed only by the `serve` start reconcile (per-org advisory lock), which audits each change; sign-in creates a missing configured row and never rewrites a role (rev 10) | N | | |
...
| .sign_in (rev 4, cp/0006) | jsonb object ≤ 4 KB: `amr`, `acr`, `idp_ipaddr`, roles, `admin_role_withheld`, `auth_time`, browser user agent; never a token or code | W | | |
...
- **Rollback:**
  ...
  - A bad signing-key version: set `tokens.signing_key_pin_version` to the previous version and redeploy. Tokens signed with the bad version stay verifiable until they expire: the pin supersedes the bad version, which leaves JWKS after max access TTL + 5 min while the pin holds (rev 9, §3.2.4). To invalidate those tokens sooner, retire it early. Keep the pin until the bad version has retired (`secret.rotated phase=retired`); removing it earlier makes the bad version sign again (`phase=reactivated`), also when a good version was rotated in but hasn't passed its activation delay.
