...
- **C1: pending the founder's own confirmation (G6 stays blocked).** On 2026-09-26 the Claude Code session (account uma.adduri@techpreneur.solutions) answered the G6 question by selecting the option 'Accept, dev/CI only'. **Claude drafted the option's wording**: accept SEC-F002-35 (b)–(d), SEC-F002-36 and SEC-F002-37 as open risks for dev and CI with synthetic identities only, to be built before F-011 or any non-dev deployment, whichever comes first. Under P6-7, C1 must be written by Ram Mohan Rao Adduri himself, so this selection records intent but doesn't satisfy C1. It is satisfied when Ram confirms in his own words: a comment on PR #46 from his own GitHub account, or a commit of the P6-7 wording. That confirmation is then cited here.
- **C2: SEC-F002-42 fixed before G7** (#47). This is a fix commitment, not a risk acceptance: the same session selected 'Fix before G7' on the same date. Recorded by Claude. Ram should confirm it in the same comment as C1.
- **Issues filed with the founder's approval:** #47–#56 (SEC-F002-42..51), #57 (D-2), D-1 added to #39.

Provenance: the decisions were asked as multiple-choice questions in the Claude Code session. Claude drafted every option's wording; the session selected the options above. No text in this subsection is the founder's own words.
