...
- **Current phase:** 5 – Development. T01–T14 and T16 are merged (#12–#15, #17, #20, #22). T18 is in three PRs, all merged: #61 (`.github/CODEOWNERS`, human merge), #62 (code-reviewer checklist, human merge) and #63 (frontend docs and `ci-duration`, agent merge). T15 (Claude Code guard) and T17 (local secret guard) are not started (no PR) and need a human merge (`.claude/**`, `.githooks`). D-F001-E2E-1 (Shift+Tab trap in `RadioGroup`, Firefox) is fixed in code in #67 (in review); once that merges it no longer blocks G6, and TC-F-001-25 stays a manual item. T20–T25 are deferred (BC-11).
- **GitHub issue:** https://github.com/AI-RAM-POC/Ralysa/issues/4
- **Branch / PR:** T13–T14 merged in #20. T18: #61 (`feat/F-001-codeowners-docs`), #62 (`feat/F-001-reviewer-checklist`), #63 (`feat/F-001-frontend-docs`). D-F001-E2E-1: #67 (`fix/F-001-radiogroup-shift-tab`), open.
- **Release:**
